...
class TestPlacesNaverMapsApiRepository {
    ...
    @DisplayName("장소명으로 도로명주소, 위도, 경도 가져오기 테스트 - log만 확인")
    @Test
    void findByPlaceName() {
        List<Place> results = this.placesRepository.findByPlaceName("비트캠프");
    ...
    @DisplayName("위도, 경도 좌표로 도로명주소, 위도, 경도 가져오기 테스트 - log만 확인")
    @Test
    void findByLatLng() {
        final int latIdx = 0;
        final int lngIdx = 1;
